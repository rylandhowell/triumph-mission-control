...
export async function extractPdfTextWithPdfJs(buffer: ArrayBuffer): Promise<string> {
  ...
  for (let i = 1; i <= pdf.numPages; i++) {
    ...
    const strings = tc.items.map((item) => ("str" in item ? item.str : "")).filter(Boolean).join(" ");
    pages.push(`\n[page ${i}]\n${strings}`);
  }
  return pages.join("\n");
}
...
function parseWindowDoorSchedules(normalized: string) {
  ...
  if (qtyHits.length) {
    ...
    return { windowDoorCount: windowQty + doorQty, source: "schedule_qty" as const };
  }
  ...
  return { windowDoorCount: windowTags.size + doorTags.size, source: "unique_types_fallback" as const };
}
...
export async function runTakeoffFromBuffers(buffers: ArrayBuffer[], heatedSqft?: number | null, underRoofSqft?: number | null) {
  ...
  for (const ab of buffers) {
    ...
    if (!hasUsefulTakeoffSignals) {
      try {
        ...
        if (ocrText.trim().length > 50) textParts.push(`\n[ocr]\n${ocrText}`);
      } catch {}
    }
  }

  return pickTakeoff(textParts.join("\n"), heatedSqft, underRoofSqft);
}
